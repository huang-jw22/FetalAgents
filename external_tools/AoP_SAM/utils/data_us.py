...
class PNGDataset(Dataset):
    def __init__(self, img_root, joint_transform: Callable = None, img_size=512, one_hot_mask: int = False) -> None:
        """
        新增 img_size 参数，用于强制缩放
        """
        self.img_root = img_root
        self.img_size = img_size  # 新增：目标尺寸
        self.one_hot_mask = one_hot_mask
        self.joint_transform = joint_transform
        
        self.img_names = sorted([f for f in os.listdir(img_root) if f.endswith('.png') or f.endswith('.jpg')])
    ...
    def __getitem__(self, i):
        ...
        # 2. 【关键修复】强制 Resize 到 img_size (例如 512x512)
        # Image 使用双线性插值，Mask 使用最近邻插值(防止产生0和1之外的小数)
