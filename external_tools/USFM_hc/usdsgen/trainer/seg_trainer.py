...
_DEFAULT_ORIG_IMG_ROOT = str(Path(__file__).resolve().parent.parent.parent / 'datasets' / 'Seg' / 'toy_dataset' / 'test_set' / 'image')

def process_prediction(pred_path, orig_img_root=_DEFAULT_ORIG_IMG_ROOT):
    ...
    # 获取原图，得到尺寸信息
    orig_name = img_name.replace("_pred", "")
    orig_img_path = os.path.join(orig_img_root, orig_name)
...
class SegTrainer(BaseTrainer):
    ...
    def test(self):
        ...
        data_loader = self.dataloader_test
        all_preds = []
        all_paths = []
        ...
        for idx, batch in enumerate(data_loader):
            ...
            all_paths.extend(batch["img_name"])

        self.logger.info(f"Inference finished, {len(all_paths)} images processed.")
        save_root = Path(self.config.output) / "inference_results"
        ...
        # 保存预测结果（按路径名）
        save_dir = Path(self.config.output) / "inference_results"
        save_dir.mkdir(parents=True, exist_ok=True)

        for img_name, pred in zip(all_paths, all_preds):
            # 创建每张图像对应的子目录，比如 ./inference_results/001/
            # img_stem = Path(img_name).stem
            # img_dir = save_root / img_stem
            # img_dir.mkdir(parents=True, exist_ok=True)
            # # 保存预测图像
            # out_path = img_dir / f"{img_stem}_pred.png"
            img_stem = Path(img_name).stem
            out_path = save_root / f"{img_stem}_pred.png"
            pred_img = (pred.numpy() * 255).astype(np.uint8)
            Image.fromarray(pred_img).save(out_path)
            # NOTE (agent mode): process_prediction() prompts for pixel size via input(),
            # which blocks non-interactive runs. Disable it when USFM_DISABLE_PROCESS_PREDICTION=1.
            if os.environ.get("USFM_DISABLE_PROCESS_PREDICTION", "0") not in ("0", "false", "False", "no", "No"):
                continue
            process_prediction(out_path)


        self.logger.info(f"All predictions saved to {save_root}")
