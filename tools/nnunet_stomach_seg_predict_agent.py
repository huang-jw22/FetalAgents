#!/usr/bin/env python3
"""
nnUNet stomach segmentation - non-interactive agent wrapper.

This script wraps the nnUNet inference pipeline for stomach segmentation,
...
Interface:
    ...
    --dataset_id: nnUNet dataset ID (default: 501)
    --configuration: nnUNet configuration (default: 2d)
    --fold: nnUNet fold (default: 0)
    --checkpoint: nnUNet checkpoint name (default: checkpoint_final.pth)

Output format (stdout):
    filename.jpg: /path/to/mask.png
"""
...
def preprocess_images(input_dir: str, proceed_dir: str, progress_every: int = 25) -> dict:
    """
    Preprocess images for nnUNet: convert to grayscale L mode and rename to *_0000.png.
    
    Returns a mapping: {nnunet_filename: original_filename}
    """
...
def postprocess_masks(
    ...
    """
    Post-process nnUNet output masks:
    - Convert pixel value 1 to 255
    - Rename from *_0000.png back to original stem
    
    Returns a mapping: {original_filename: mask_path}
    """
    Path(final_dir).mkdir(parents=True, exist_ok=True)
    results = {}
    
    # Build reverse mapping: nnunet_base -> original_name
    # nnunet outputs without _0000 suffix
    reverse_map = {}
    for nnunet_name, orig_name in name_mapping.items():
        # nnunet_name is like "abdomcirc_000_0000.png"
        # nnunet output is like "abdomcirc_000.png"
    ...
    for idx, fpath in enumerate(out_files, start=1):
        ...
        try:
            ...
            # Save with original stem but _prediction.png suffix
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="nnUNet stomach segmentation - agent entrypoint"
    )
    parser.add_argument(
        "--data_path",
        required=True,
    ...
    parser.add_argument(
        "--out_dir",
        default="",
        help="Output directory for masks (default: auto-generated)",
    )
    parser.add_argument(
        "--nnunet_predict",
        default=DEFAULT_NNUNET_PREDICT,
        help="Path to nnUNetv2_predict executable",
    )
    parser.add_argument(
        "--dataset_id",
        type=int,
    ...
    parser.add_argument(
        ...
        help="nnUNet checkpoint name (default: checkpoint_final.pth)",
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=600,
        help="Timeout in seconds for nnUNet prediction (default: 600)",
    )
    parser.add_argument(
        "--progress_every",
        type=int,
        default=25,
        help="Print progress every N images during pre/post-processing (default: 25)",
    )
    parser.add_argument(
        "--keep_temp",
        action="store_true",
    ...
    if not out_dir:
        ...
        out_dir = str(agent_root / "outputs_agent" / "stomach_seg" / "nnunet")
    Path(out_dir).mkdir(parents=True, exist_ok=True)
    
    # Create temporary directories for nnUNet workflow
    temp_base = tempfile.mkdtemp(prefix="nnunet_stomach_")
    ...
    try:
        ...
        name_mapping = preprocess_images(
            args.data_path,
            proceed_dir,
        ...
        print(f"Preprocessed {len(name_mapping)} images", file=sys.stderr, flush=True)
        ...
        # Output in expected format: filename: mask_path
